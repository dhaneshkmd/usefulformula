...
const srcDocxPath: string = srcDocx;
...
(async () => {
  // 1) DOCX → Markdown (cast to any to satisfy TS)
  let md = "";
  try {
  ...
  for (const raw of lines) {
    ...
    if (/^[A-Za-z].{0,60}:$/.test(line)) { currentSub = line.replace(/:$/, "").trim(); continue; }
  ...
  // 3) Merge with usefulformula.json
  let base: Item[] = [];
  if (fs.existsSync(mergedOut)) {
    try { base = JSON.parse(fs.readFileSync(mergedOut, "utf8")); } catch {}
  }
